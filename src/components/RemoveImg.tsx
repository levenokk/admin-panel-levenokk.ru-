...
const RemoveImg: React.FC<RemoveImgProps> = ({ id }) => {
  ...
  const removeImg = () => {
    remove({ variables: { id } })
      .then(() => {
        dispatch(notificationAction('Зображення успішно видалено', Note.success))
        handleClose()
      })
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="alert-dialog-title">Ви дійсно хочете видалити зображення ?</DialogTitle>
        <DialogContent>
